...
class CapsNet(object):
    def __init__(self, is_training=True):
        ...
        self.Y = tf.placeholder(tf.float32, shape=(cfg.batch_size, 10))
        self.build_arch()
        self.loss()
        # t_vars = tf.trainable_variables()
        self.optimizer = tf.train.AdamOptimizer()
        self._train_op = self.optimizer.minimize(self.margin_loss)  # var_list=t_vars)
    ...
    def predict(self, batch_xs):
        result = self.sess.run(self.argmax_idx, feed_dict={self.X: batch_xs}) 
        return result 
    def train(self, batch_xs, batch_ys):
         _,loss,acc = self.sess.run([self._train_op, self.margin_loss, self.accuracy],feed_dict={self.X: batch_xs, self.Y: batch_ys})
         return loss, acc
